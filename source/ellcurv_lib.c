...
int ell_c_fact(mpz_t factors[], const mpz_t n, const mpz_t b1, const mpz_t b2, gmp_randstate_t state)
{
	...
	for (i = 0; i < MAX_ITER ; i++)
	{
		...
		GMP_DEBUG_STAMP("testing p: (%Zd, %Zd)\non curv: Y^2 = X^3 +%ZdX + %Zd\n", p.x, p.y, e.A, e.B);
		...
		{	
			...
			if (ell_p_fact_fase_2(factors[0], e, r, b1, b2, &temp, &p_temp, &rep))
			{
				fase_found = 2;
				break;
			}		
...
int ell_p_mul(mpz_t factor, const ell_c e, ell_p *r, const ell_p p, const mpz_t k, mpz_temp *temp, ell_rep *p_temp)
{
	ell_p *pow = (p_temp->p);
...
int create_r_difference(ell_rep *rep, mpz_t factor, const ell_c e, const ell_p p, unsigned long int start, unsigned long int end, 
						mpz_temp *temp)
{
	...
	{
		if (ell_add(e, &(rep->p[empty_i]), rep->p[empty_i - 1], rep->p[0], temp))
		{
...
int ell_p_fact_fase_2(mpz_t factor, const ell_c e, const ell_p r, const mpz_t b1, const mpz_t b2, mpz_temp *temp, ell_rep *p_temp, 
						ell_rep *rep)
{
	...
	if (!ell_p_mul(factor, e, q, r, *prime, temp, p_temp))  // q = r * qo
		while (mpz_cmp(*prime, b2) != 1)
		{
			...
			if (diff_ui > max_diff)
			{
				if (create_r_difference(rep, factor, e, r, max_diff, diff_ui + 4, temp))
					goto end;
				max_diff = diff_ui + 4;	
			}
