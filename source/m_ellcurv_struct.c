...
void m_ellp_temp_init2(m_ellp_temp *temp, unsigned long int lenght, mp_bitcnt_t size)
{
	temp->index = 0;
	unsigned int i;
	temp->p = allocate(sizeof(m_ellp) * lenght);
...
void m_ellp_rep_init2(m_ellp_rep *rep, unsigned long int lenght, mp_bitcnt_t size)
{
	unsigned int i;
	rep->p = allocate(sizeof(m_ellp) * lenght);
	rep->lenght = lenght;
