...
static void test_fact_times(char *tent_s, char *cifer_s, char *b_s, char *path_t, char *path_f);
static void test_fact(FILE *log_t, FILE *log_f, unsigned int tent, unsigned long int cifer, unsigned long b1, unsigned long b2, 
...
static void test_fact(FILE *log_t, FILE *log_f, unsigned int tent, unsigned long cifer, unsigned long b1, unsigned long b2, 
						gmp_randstate_t state)
{
	...
	for (unsigned int i = 0; i < tent; i++)
	{
		...
		if (res != ELL_FACT_NOT_FOUND)
		{
			gmp_fprintf(log_f, "n = %Zd\tf1 = %Zd\tf2 = %Zd\tFASE = %d\t ITER = %d\n", n, fact[0], fact[1], 
						ell_fact_FASE(res, DEFAULT_COEFF), ell_fact_ITER(res, MAX_ITER));
	...
	gmp_fprintf(log_t, "%lu,%lu,%lu,%lu,%lu.%lu\n", cifer, b1, b2, tot_iter, diff.tv_sec, diff.tv_nsec / 1000000);	
...
void test_mfase2(char *n_s, char *b1_s, char *b2_s)
{
	...
	mm_ell_fase2(g_r, b1, b2, e_r.C2, &p_r, rep.p, beta.v, rep.lenght, vdiff, &mdata, &p_temp, &temp);
	get_current_time(&time2);
	timespec_diff(&time1, &time2, &time3);
