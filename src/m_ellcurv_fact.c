...
void m_ell_fact(mpz_t fact, gmp_randstate_t state, mpz_t e_C2, const m_fact_param *param, m_ellp_rep *rep, 
				mpz_rep *beta, mpz_temp *temp, m_ellp_temp *p_temp, unsigned int *iter, int *fase_found)
{
	#define n param->n
	m_ellp *p, *r;
	mpz_t *gcd;
	...
	for (*iter = 0; *iter < param->max_iter; (*iter)++)
		...
		else
		{
			...
			m_ell_fase2(*gcd, param->b1, param->b2, n, e_C2, r, *rep, *beta, param->vdiff, p_temp, temp);
...
void mm_ell_fact(mpz_t fact, gmp_randstate_t state, mpz_t e_C2, const mm_fact_param *param, m_ellp_rep *rep, 
				mpz_rep *beta, mpz_temp *temp, m_ellp_temp *p_temp, unsigned int *iter, int *fase_found)
{
...
void *fact_tjob(void *arg)
{
	...
	ell_fact(td->fact, td->state, td->e_C2, td->const_param, &(td->rep), &(td->beta), &(td->temp), &(td->p_temp), 
				&(td->iter_done), &(td->fase_found));
	lock(td->mtx);
	if (td->fase_found != -1)
	{
...
//HIGH_LEVEL FUNCT
int factorize(mpz_t factors[], const mpz_t n, unsigned long b1, unsigned long b2, unsigned long max_iter)
{
	...
	#else
	ell_fact(factors[0], td[0].state, td[0].e_C2, &param, &(td[0].rep), &(td[0].beta), &(td[0].temp), &(td[0].p_temp),
			&iter_done, &fase_found);
	#endif	
